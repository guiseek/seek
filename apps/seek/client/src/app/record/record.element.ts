...
import './record.element.scss'

declare global {
  interface HTMLElementTagNameMap {
    'seek-record': RecordElement
  }
}

@Element({
  selector: 'seek-record',
  template: html`
    <section>
...
export class RecordElement extends HTMLElement implements OnConnect {
  ...
  async init(constraints: MediaStreamConstraints) {
    try {
      this.handleSuccess(await navigator.mediaDevices.getDisplayMedia(constraints))
    } catch (err) {
      this.error.innerHTML = `navigator.getDisplayMedia error:${err.toString()}`
    }
  }
}
